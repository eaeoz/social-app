...
async function deleteInactiveUsers(protectedUsername, inactiveDays) {
  let client;
  try {
    ...
    console.log('⚠️  WARNING: This will delete ALL users (except protected) who have not logged in since this date!');
    console.log('⏳ Starting in 5 seconds... Press Ctrl+C to cancel\n');
    
    // Give user time to cancel
    await new Promise(resolve => setTimeout(resolve, 5000));
    
    // Find all users except the protected one who haven't been active since cutoff date
    ...
    usersToDelete.forEach((user, index) => {
      const lastSeenDate = user.lastSeen ? new Date(user.lastSeen).toLocaleDateString() : 'Never';
      const daysInactive = user.lastSeen ? Math.floor((Date.now() - new Date(user.lastSeen)) / (1000 * 60 * 60 * 24)) : 'N/A';
      console.log(`   ${index + 1}. ${user.username} (Last seen: ${lastSeenDate}, ${daysInactive} days ago)`);
    });
    ...
    for (let i = 0; i < usersToDelete.length; i++) {
      ...
      console.log(`\n[${i + 1}/${usersToDelete.length}] Deleting user: ${user.username} (Inactive: ${daysInactive} days)`);
    ...
    // Print final summary
    console.log('\n' + '='.repeat(70));
    console.log('🎉 INACTIVE USER DELETION COMPLETE - Final Summary:');
    console.log('='.repeat(70));
    console.log(`🔒 Protected user: ${protectedUsername}`);
    console.log(`📅 Inactivity threshold: ${inactiveDays} days`);
    ...
    console.log('='.repeat(70));
    console.log(`✅ All inactive users (not active for ${inactiveDays}+ days, except protected) have been permanently deleted!`);
    console.log('='.repeat(70) + '\n');
    
    process.exit(0);
  } catch (error) {
    console.error('❌ Error during inactive user deletion:', error);
    process.exit(1);
  } finally {
    if (client) {
      await client.close();
...
const inactiveDays = parseInt(process.argv[2]);
const protectedUsername = process.argv[3] || 'sedat';

// Validate input
if (isNaN(inactiveDays) || inactiveDays <= 0) {
  console.error('\n❌ Error: Invalid number of days');
  console.log('\nUsage: node server/utils/deleteAllUsersExceptNotActiveDays.js <days> [protectedUsername]');
  console.log('\nExamples:');
  console.log('  node server/utils/deleteAllUsersExceptNotActiveDays.js 90');
  console.log('  node server/utils/deleteAllUsersExceptNotActiveDays.js 90 sedat');
  console.log('  node server/utils/deleteAllUsersExceptNotActiveDays.js 30 admin\n');
  process.exit(1);
}

console.log('\n' + '⚠️'.repeat(35));
console.log('⚠️  DANGER: INACTIVE USER DELETION UTILITY  ⚠️');
console.log('⚠️'.repeat(35));
console.log(`\nThis will DELETE ALL USERS who have not logged in for ${inactiveDays}+ days`);
console.log(`Protected user "${protectedUsername}" will NOT be deleted`);
...
deleteInactiveUsers(protectedUsername, inactiveDays);
